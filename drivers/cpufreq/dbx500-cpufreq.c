...
static int freq_table_len,last_idx=0;
				...
				unsigned int relation)
{
	...
	if (last_idx < idx) {
 	prcmu_abb_write(AB8500_REGU_CTRL2, AB8500_VBB_REG, &freq_table[idx].vbb_raw, 1);
 	prcmu_abb_write(AB8500_REGU_CTRL2, freq_table[idx].varm_sel, &freq_table[idx].varm_raw, 1);
	//udelay(80);
 	db8500_prcmu_write(PRCMU_ARMPLL_REG, freq_table[idx].raw_freq);
 	} else {
 	db8500_prcmu_write(PRCMU_ARMPLL_REG, freq_table[idx].raw_freq);
	//udelay(20);
 	prcmu_abb_write(AB8500_REGU_CTRL2, freq_table[idx].varm_sel, &freq_table[idx].varm_raw, 1);
	...
	prcmu_qos_update_requirement(PRCMU_QOS_DDR_OPP, "cpufreq",(signed char)freq_table[idx].ddr_opp);
	prcmu_qos_update_requirement(PRCMU_QOS_APE_OPP, "cpufreq",(signed char)freq_table[idx].ape_opp);
	last_idx=idx;
	/*db8500_prcmu_set_arm_opp(freq_table[idx].opp);
	if (freq_table[idx].frequency == 1000000)
			db8500_prcmu_set_arm_opp(ARM_MAX_OPP);
	if (freq_table[idx].is_overclocked == 1) {
	prcmu_abb_write(AB8500_REGU_CTRL2, freq_table[idx].varm_sel, &freq_table[idx].varm_raw, 1);
	prcmu_abb_write(AB8500_REGU_CTRL2, AB8500_VBB_REG, &freq_table[idx].vbb_raw, 1);
	prcmu_qos_update_requirement(PRCMU_QOS_DDR_OPP, "cpufreq",(signed char)freq_table[idx].ddr_opp);
	prcmu_qos_update_requirement(PRCMU_QOS_APE_OPP, "cpufreq",(signed char)freq_table[idx].ape_opp);
	db8500_prcmu_write(PRCMU_ARMPLL_REG, freq_table[idx].raw_freq);//} else*/
