				    ...
				    ssp_clock_params * clk_freq)
{
	...
	/* Parent clock for SPI(per1clk) changes dynamically between 66MHz
	 * and 133MHz depending on APE OPP level, along with that, derived SPI
	 * clock is changed. To be sure that we really set the maximum SPI
	 * clock rate, we need to take max freqency of parent clock. One way to
	 * do this is to request APE 100% before getting clock rate.
	 */
	prcmu_qos_add_requirement(PRCMU_QOS_APE_OPP, "ssp-pl022", 100);
	rate = clk_get_rate(pl022->clk);
	prcmu_qos_remove_requirement(PRCMU_QOS_APE_OPP, "ssp-pl022");
